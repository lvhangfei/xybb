...
public class Es {
    ...
    public void buildIndexSysDm22() throws Exception {
        ...
        IndexResponse response = client.prepareIndex("productindex", "prindextype", "jk8231")
                .setSource(XContentFactory.jsonBuilder()
                                ...
                                .field("onSale", true)
                                .field("type", 2)
                                .field("createDate", TimeUtil.getNowTime())
                                        //该字段在上面方法中的mapping中没有定义,所以该字段的属性使用es默认的.
                                .field("dfsfs", "哈哈")
                                .endObject()
                )
    ...
    public Client buildClient() {
        Settings settings = ImmutableSettings.settingsBuilder()
                .put("client.transport.sniff", true).put("cluster.name", "xybb").build();
        //添加节点
    ...
    public void exm() {
        ...
        QueryBuilder qb2 = QueryBuilders.boolQuery()
                ...
                .should(new QueryStringQueryBuilder("3").field("dfsfs"))
                        //关于QueryStringQueryBuilder及termQuery等的使用可以使用es插件head来进行操作体会个中query的不同
                .must(QueryBuilders.termQuery("dfsfs", "里"));
